...
export function AiReport({
  text, title = "Diagnóstico de IA", subtitle, meta, onRefresh, refreshing,
}: {
  ...
  function downloadPdf() {
    ...
    if (!w) return;
    w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${safe(title)} — SAPLINK</title>
    <style>
      *{box-sizing:border-box} body{font-family:Arial,Helvetica,sans-serif;color:#1a1527;margin:0;padding:48px 56px;line-height:1.55}
      ...
      <div class="brand"><span class="logo">◆ SAPLINK</span><span class="tag">Relatório gerado por IA</span></div>
      <h1>${safe(title)}</h1>${subtitle ? `<p class="sub">${safe(subtitle)}</p>` : ""}
      ${metaHtml ? `<div class="meta">${metaHtml}</div>` : ""}
      <div class="content">${bodyHtml}</div>
      <div class="foot"><span>SAPLINK · saplink.com.br</span><span>Emitido em ${now}</span></div>
    ...
    <div className="rounded-xl border border-purple-500/20 bg-gradient-to-b from-purple-500/[0.06] to-transparent overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-purple-500/15 bg-purple-500/[0.06] flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          {onRefresh && <button onClick={onRefresh} className="text-xs text-[#9b95ad] hover:text-purple-300 underline">{refreshing ? "Analisando…" : "Refazer análise"}</button>}
