...
export default function PortalPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-[#9b95ad]">Carregando...</div>;
  if (error || !data) return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f0b1a] text-center px-4">
      <div>
        <h1 className="text-xl font-bold text-[#e2e0ea]">Portal indisponível</h1>
        <p className="text-[#9b95ad] mt-2">Este link de portal não existe ou foi desativado.</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-transparent text-[#e2e0ea]">
      <header className="border-b border-white/[0.08] px-6 py-4 flex items-center justify-between">
        ...
        <span className="text-sm text-[#9b95ad]">Portal de saúde · {data.client.name}</span>
      </header>

      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        <div>
          <h1 className="text-2xl font-bold">{data.client.name}</h1>
          <p className="text-[#9b95ad] text-sm mt-1">Visão de integrações SAP em tempo real · somente leitura</p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card label="Health score" value={`${data.client.healthScore}`} accent={accent} />
          <Card label="Uptime médio" value={`${data.summary.avgUptime}%`} accent="#34d399" />
          <Card label="Integrações" value={`${data.summary.integrations}`} accent="#e2e0ea" />
          <Card label="Incidentes abertos" value={`${data.summary.openIncidents}`} accent={data.summary.openIncidents ? "#f87171" : "#34d399"} />
        </div>

        <section>
          <h2 className="text-sm font-semibold text-[#9b95ad] uppercase tracking-wider mb-2">Integrações</h2>
            ...
            {data.integrations.map((i, idx) => (
              <div key={idx} className="flex items-center justify-between px-4 py-3 border-b border-white/[0.04] last:border-0">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[#9b95ad]">{i.type} · uptime {i.uptime}% · {i.latency}ms</p>
                </div>
                <span className={`text-sm font-semibold ${statusCls(i.status)}`}>{i.status}</span>
              </div>
            ))}
            {data.integrations.length === 0 && <p className="px-4 py-3 text-sm text-[#9b95ad]">Nenhuma integração cadastrada.</p>}
          </div>
        </section>

        {data.incidents.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-[#9b95ad] uppercase tracking-wider mb-2">Incidentes abertos</h2>
            <div className="space-y-2">
              {data.incidents.map((a, idx) => (
                <div key={idx} className="bg-[#1a1527] border border-white/[0.08] rounded-lg px-3 py-2">
                  ...
                  <p className="text-xs text-[#9b95ad] mt-0.5">{new Date(a.createdAt).toLocaleString("pt-BR")}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <footer className="text-center text-xs text-[#6b6580] pt-6">
          Monitorado por {data.consultancy.name} · powered by SAPLINK
        </footer>
      </main>
    </div>
  );
