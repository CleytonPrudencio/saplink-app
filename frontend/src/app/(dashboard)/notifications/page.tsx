          ...
          {channels.map((c) => (
            <div key={c.id} className="flex items-center justify-between gap-3 bg-[#0f0b1a] rounded-lg px-3 py-2 flex-wrap">
              ...
              <div className="flex gap-2">
                <button onClick={() => onTest(c.id)} disabled={busy === c.id} className="text-xs px-2.5 py-1 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] cursor-pointer disabled:opacity-40">{busy === c.id ? "..." : t.test}</button>
                <button onClick={() => onDelete(c.id)} className="text-xs px-2.5 py-1 rounded-lg bg-white/[0.06] hover:bg-rose-500/20 hover:text-rose-300 cursor-pointer">{t.remove}</button>
              </div>
            </div>
          ))}
