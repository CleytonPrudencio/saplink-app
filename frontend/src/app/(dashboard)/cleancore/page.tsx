    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-[#1a1527] rounded-xl p-6 border border-white/[0.08] text-center">
          ...
          <div className="text-sm text-[#9b95ad] mt-2">Score geral da carteira</div>
        ...
        <div className="lg:col-span-2 bg-[#1a1527] rounded-xl p-5 border border-white/[0.08]">
          <h2 className="text-sm font-semibold text-[#9b95ad] mb-3">Score por cliente</h2>
          <div className="space-y-2">
            {(data?.perClient || []).map((c: any) => (
              <div key={c.clientId} className="flex items-center gap-3">
                ...
                <span className="text-xs text-[#9b95ad] w-20 text-right">{c.items} itens</span>
              </div>
            ))}
            {(!data?.perClient || data.perClient.length === 0) && <p className="text-sm text-[#9b95ad]">Sem dados ainda.</p>}
          </div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto border border-white/[0.08] rounded-xl">
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[#9b95ad] border-b border-white/[0.08] bg-white/[0.02]">
            <th className="px-3 py-2 font-medium">Categoria</th><th className="px-3 py-2 font-medium">Objeto</th>
            <th className="px-3 py-2 font-medium">Severidade</th><th className="px-3 py-2 font-medium text-right">-pts</th>
            <th className="px-3 py-2 font-medium">Recomendação</th><th className="px-3 py-2 font-medium">Cliente</th>
          </tr></thead>
          <tbody>
            ...
            {(!data?.items || data.items.length === 0) && <tr><td colSpan={6} className="px-3 py-6 text-center text-[#9b95ad]">Core limpo ou sem dados ainda.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
