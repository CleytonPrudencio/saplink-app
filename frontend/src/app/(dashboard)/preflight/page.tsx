...
export default function PreflightPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">🧨 Pré-voo de mudança</h1>
        <p className="text-[#9b95ad] text-sm mt-1">Antes de um transport ir pra produção, veja o <b>raio de impacto</b> (interfaces, processos, R$/h em risco) e o <b>score de risco</b> — para testar o que importa primeiro.</p>
      </div>

      {loading ? <div className="text-[#9b95ad]">Carregando...</div> : transports.length === 0 ? (
        <div className="bg-[#1a1527] rounded-xl p-8 border border-white/[0.08] text-center text-[#9b95ad]">Sem transports recentes. Conecte o agente (STMS) para ver as mudanças.</div>
      ) : (
        <>
          <select value={sel} onChange={(e) => setSel(e.target.value)} className="bg-[#1a1527] border border-white/[0.1] rounded-lg px-3 py-2 text-sm w-full max-w-xl">
            {transports.map((t) => <option key={t.id} value={t.id}>{t.trNumber} — {t.description || "(sem descrição)"} → {t.target} ({t.client})</option>)}
          </select>
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="rounded-2xl p-5 border" style={{ borderColor: r.riskScore >= 70 ? "#f8717140" : r.riskScore >= 40 ? "#fbbf2440" : "#34d39940", background: "rgba(255,255,255,0.02)" }}>
                  <div className="text-xs text-[#9b95ad]">Score de risco da mudança</div>
                ...
                <Stat label="Interfaces no raio" value={r.affected.interfaces.length} sub="catálogo do cliente" />
                <Stat label="R$/h em risco" value={brl(r.affected.atRiskPerHourCents)} sub={`${r.affected.processes.length} processo(s) · ${r.affected.integrations.length} integrações`} amber />
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-[#1a1527] rounded-xl p-5 border border-white/[0.08]">
                  <h2 className="font-semibold mb-3">Processos & integrações afetados</h2>
                  <div className="flex flex-wrap gap-1.5 mb-3">{r.affected.processes.map((p: string) => <span key={p} className="text-xs px-2 py-1 rounded-full bg-purple-500/15 text-purple-200">{p}</span>)}{r.affected.processes.length === 0 && <span className="text-xs text-[#6b6580]">Sem processo classificado nas integrações.</span>}</div>
                ...
                <div className="bg-[#1a1527] rounded-xl p-5 border border-emerald-500/20">
                  <h2 className="font-semibold mb-3">✅ Plano de teste recomendado</h2>
