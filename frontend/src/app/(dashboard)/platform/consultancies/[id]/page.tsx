...
export default function ConsultancyDetailPage() {
  ...
  useEffect(() => {
    ...
    setClient(null);
    getPlatformClientDetail(clientId).then(setClient).catch(() => notify("Erro ao carregar cliente.", "error"));
  }, [clientId]);
  ...
  async function saveConsultancy() {
    ...
    try {
      ...
      notify("Cadastro atualizado.", "success"); setEditCons(false); await load();
    } catch { notify("Falha ao salvar.", "error"); } finally { setBusy(false); }
  }
  ...
  async function saveUser() {
    ...
    try {
      ...
      notify("Usuário atualizado.", "success"); setEditUser(null); await load();
    } catch (e: any) { notify(e?.response?.data?.error || "Falha ao salvar.", "error"); } finally { setBusy(false); }
  }

  async function resetPwd(userId: string, email: string) {
    try {
      const r = await platformResetPassword(userId);
      notify(`Senha temporária de ${email}: ${r.tempPassword}`, "success");
    } catch { notify("Falha ao resetar senha.", "error"); }
  }
  ...
  async function toggleAccess(suspend: boolean) {
    ...
    try {
      ...
      notify(suspend ? "Acesso suspenso." : "Acesso reativado.", "success"); await load();
    } catch { notify("Operação falhou.", "error"); } finally { setBusy(false); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={editCons} onClose={() => setEditCons(false)} title="Editar cadastro da consultoria">
        <div className="space-y-4">
          <Field label="Nome"><input className={inputClass} value={consForm.name} onChange={(e) => setConsForm({ ...consForm, name: e.target.value })} /></Field>
          <Field label="CNPJ"><input className={inputClass} value={consForm.cnpj} onChange={(e) => setConsForm({ ...consForm, cnpj: e.target.value })} placeholder="00.000.000/0001-00" /></Field>
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setEditCons(false)} className="px-4 py-2 rounded-lg text-sm bg-white/[0.06]">Cancelar</button>
            <button disabled={busy} onClick={saveConsultancy} className="px-4 py-2 rounded-lg text-sm font-semibold bg-purple-500 text-white disabled:opacity-40">Salvar</button>
          </div>
        </div>
      </Modal>

      {/* ===== MODAL: editar usuário ===== */}
      <Modal open={!!editUser} onClose={() => setEditUser(null)} title="Gerenciar usuário">
        {editUser && (
          <div className="space-y-4">
            <Field label="Nome"><input className={inputClass} value={userForm.name} onChange={(e) => setUserForm({ ...userForm, name: e.target.value })} /></Field>
            <Field label="E-mail"><input className={inputClass} value={userForm.email} onChange={(e) => setUserForm({ ...userForm, email: e.target.value })} /></Field>
            <Field label="Papel">
              <select className={inputClass} value={userForm.role} onChange={(e) => setUserForm({ ...userForm, role: e.target.value })}>
                <option value="CONSULTANCY_ADMIN">Admin da consultoria</option>
                <option value="CONSULTANCY_USER">Usuário</option>
              </select>
            </Field>
            <div className="flex justify-between items-center pt-2">
              <button onClick={() => resetPwd(editUser.id, editUser.email)} className="px-3 py-2 rounded-lg text-sm bg-amber-500/15 text-amber-300 hover:bg-amber-500/25">Resetar senha</button>
              <div className="flex gap-2">
                <button onClick={() => setEditUser(null)} className="px-4 py-2 rounded-lg text-sm bg-white/[0.06]">Cancelar</button>
                <button disabled={busy} onClick={saveUser} className="px-4 py-2 rounded-lg text-sm font-semibold bg-purple-500 text-white disabled:opacity-40">Salvar</button>
              </div>
            </div>
          </div>
        )}
      ...
      <Modal open={!!clientId} onClose={() => setClientId(null)} title={client ? client.name : "Carregando..."} size="lg">
        {!client ? (
          <div className="text-[#9b95ad]">Carregando...</div>
        ) : (
          <div className="space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div><p className="text-[#9b95ad]">Health score</p><p className="font-medium">{client.healthScore}</p></div>
              <div><p className="text-[#9b95ad]">CNPJ</p><p className="font-medium">{client.cnpj || "-"}</p></div>
              <div><p className="text-[#9b95ad]">Integrações</p><p className="font-medium">{client.integrations.length}</p></div>
              <div><p className="text-[#9b95ad]">Alertas abertos</p><p className="font-medium">{client.openAlerts.length}</p></div>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-[#9b95ad] mb-2">Integrações</h4>
              <div className="space-y-2">
                    ...
                    {openIntg === i.id && (
                      <div className="px-3 pb-3 border-t border-white/[0.05] text-sm space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2">
                          <div><p className="text-[#9b95ad] text-xs">Latência</p><p>{i.latency} ms</p></div>
                          <div><p className="text-[#9b95ad] text-xs">Erro</p><p>{i.errorRate}%</p></div>
                          <div><p className="text-[#9b95ad] text-xs">Uptime</p><p>{i.uptime}%</p></div>
                        </div>
                        {i.config && (
                          <div>
                            <p className="text-[#9b95ad] text-xs mb-1">Configuração (segredos mascarados)</p>
                ...
                {client.integrations.length === 0 && <p className="text-sm text-[#9b95ad]">Nenhuma integração.</p>}
              </div>
            </div>

            {client.openAlerts.length > 0 && (
              <div>
                <h4 className="text-sm font-semibold text-[#9b95ad] mb-2">Alertas abertos</h4>
                <div className="space-y-2">
