...
import { getConsultancyDetail, suspendConsultancy, activateConsultancy } from "@/lib/api";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1a1527] rounded-xl p-6 border border-white/[0.08]">
          ...
          <div className="space-y-2">
            {c.users.map((u: any) => (
              <div key={u.id} className="flex justify-between text-sm bg-[#0f0b1a] rounded-lg px-3 py-2">
                <span>{u.name} <span className="text-[#9b95ad]">· {u.email}</span></span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-white/[0.06]">{u.role}</span>
              </div>
            ))}
